...
github_repo="blue1244/V2Ray_ws-tls_bash_onekey"
...
apt_install_any() {
    ...
    for package in "$@"; do
        if apt-get install -y "${package}"; then
            echo -e "${OK} ${GreenBG} ${description}: ${package} 安装完成 ${Font}"
            return 0
        fi
    done

    echo -e "${Error} ${RedBG} ${description} 安装失败: $* ${Font}"
    return 1
}
...
modify_nginx_port() {
    ...
    sed -i "/listen .* ssl;$/c \\\tlisten ${port} ssl;" ${nginx_conf}
...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N https://raw.githubusercontent.com/${github_repo}/${github_branch}/v2ray.sh
...
nginx_install() {
    ...
    wget -nc https://nginx.org/download/nginx-${nginx_version}.tar.gz -P ${nginx_openssl_src}
    judge "Nginx 下载"
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget https://raw.githubusercontent.com/${github_repo}/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget https://raw.githubusercontent.com/${github_repo}/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_inbound_port
    modify_UUID
}
...
acme_cron_update() {
    wget -N -P /usr/bin "https://raw.githubusercontent.com/${github_repo}/${github_branch}/ssl_update.sh"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/${github_repo}/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N https://raw.githubusercontent.com/${github_repo}/${github_branch}/install.sh
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/${github_repo}/${github_branch}/v2ray.sh)
        ;;
    4)
